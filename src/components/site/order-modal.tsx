...
type Step = "instructions" | "confirming" | "delivered";
...
export function OrderModal({ product, open, onOpenChange }: OrderModalProps) {
  ...
  const orderRef = `CMD-${product.id.slice(-6).toUpperCase()}-${Date.now().toString().slice(-5)}`;
  ...
  const handleConfirmPayment = async () => {
    setConfirming(true);
    // Simulation de vérification du paiement (à remplacer par vraie API Wave plus tard)
  ...
  const copyDelivery = () => {
    if (product.deliveryContent) {
      navigator.clipboard.writeText(product.deliveryContent);
    }
  };
      ...
      <DialogContent className="sm:max-w-[480px] max-h-[90vh] overflow-y-auto">
        {step === "delivered" ? (
          ...
          <div>
            ...
            {product.instantDelivery && product.deliveryContent ? (
              <>
                ...
                <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-xs text-amber-800 mb-4">
                  ...
                  informations. Pour tout problème, contactez-nous sur Instagram{" "}
                  <a
              ...
              <div className="rounded-xl border-2 border-sky-200 bg-sky-50 p-4 mb-4">
                ...
                <p className="text-sm text-slate-700">
                  Votre paiement a bien été enregistré. Votre commande sera
                  livrée dans un délai de{" "}
                  <strong>{product.deliveryTime || "24 à 48h"}</strong>.
                </p>
                <p className="text-sm text-slate-700 mt-2">
                  Vous serez notifié(e) dès que votre commande est prête. Pour
          ...
          // ========== ÉTAPE 1+2 : INSTRUCTIONS + PAIEMENT ==========
          <>
            <DialogHeader>
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-center text-slate-500 flex items-center justify-center gap-1">
                ...
                La confirmation déclenche la livraison automatique
                {product.instantDelivery ? " ⚡" : ""}.
              </p>
            </div>
          </>
